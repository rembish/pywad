...
def configure(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        nargs="?",
        default=None,
        help="output PNG path (default: <PATCH>.png)",
    )
    p.add_argument(
...
def run(args: argparse.Namespace) -> None:
    patch_name = args.patch.upper()
    output: str = args.output or f"{patch_name}.png"
    with open_wad(args) as wad:
        pic = wad.get_picture(patch_name)
        if pic is None:
            print(f"Patch '{patch_name}' not found.", file=sys.stderr)
            sys.exit(1)
